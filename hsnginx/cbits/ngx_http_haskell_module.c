...
static char *ngx_http_haskell(ngx_conf_t *cf, void *post, void *data);

static ngx_conf_post_handler_pt ngx_http_haskell_p = ngx_http_haskell;
...
ngx_http_haskell_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_haskell_loc_conf_t));
    if (conf == NULL) {
        return NULL;
    }
...
HsBool haskellso_init(void) {
  int argc = 1;
  static char *argv[] = {"nginx",0};
...
static ngx_command_t ngx_http_haskell_commands[] = {
    { ngx_string("hello"),
      ...
      offsetof(ngx_http_haskell_loc_conf_t, name),
      &ngx_http_haskell_p },
...
static ngx_str_t haskell_string;
 ...
 */
static ngx_int_t
ngx_http_haskell_handler(ngx_http_request_t *r)
{
    ngx_int_t    rc;
    ngx_buf_t   *b;
    ngx_chain_t  out;
    ...
    char *p = malloc(1024);
    p[0] = '\0';
    char * b1 = (char *)reverseIt(r->pool, haskell_string.data, haskell_string.len);
    strncat(p, b1, haskell_string.len);
    int l = strlen(p);
    snprintf(p+l,100, ": %i",i);
    l = strlen(p);
    printf("%s",p);
    // <<
    ...
    if (r->method == NGX_HTTP_HEAD) {
        ...
        r->headers_out.content_length_n = l; //haskell_string.len;
    ...
    b->pos = p; //haskell_string.data;
    b->last = p + l; //haskell_string.data + haskell_string.len;
 ...
 */
static char *
ngx_http_haskell(ngx_conf_t *cf, void *post, void *data)
{
    ...
    ngx_str_t  *name = data; // i.e., first field of ngx_http_haskell_loc_conf_t
    
    if (ngx_strcmp(name->data, "") == 0) {
        return NGX_CONF_ERROR;
    }
    haskell_string.data = name->data;
    haskell_string.len = ngx_strlen(haskell_string.data);
