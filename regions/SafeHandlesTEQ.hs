...
instance (Monad m1, Monad m2, MonadRaise' m1 m2 (m1 == m2)) => MonadRaise m1 m2 where
   lifts = lifts' (undefined :: Proxy (m1 == m2))

class MonadRaise' m1 m2 (c::Bool) where
   lifts' :: proxy c -> m1 a -> m2 a

instance (m1 ~ m2) => MonadRaise' m1 m2 True where
   lifts' _ = id

instance (MonadRaise m1 m2) => MonadRaise' m1 (IORT s m2) False where
